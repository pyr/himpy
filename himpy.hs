...
collect_profiles :: TChan ([Metric]) -> TChan (String) -> HimpyHost -> HIndex -> IO ()
collect_profiles chan logchan (Host host comm (prof:profs)) index = do
  recipe prof chan logchan (Host host comm [prof]) index
  collect_profiles chan logchan (Host host comm profs) index
collect_profiles chan logchan (Host host _ []) index = do
  log_info logchan $ "ticking for host: " ++ host
  threadDelay 10000000

start_collector ::  TChan ([Metric]) -> TChan (String) -> HimpyHost -> HIndex -> IO ()
start_collector chan logchan host index =
  void $ forkIO $ forever $ collect_profiles chan logchan host index

start_collectors :: TChan ([Metric]) -> TChan (String) -> HimpyConfig -> HIndex -> IO ()
start_collectors chan logchan (Hosts x y z (host:hosts) _) index = do
  start_collector chan logchan host index
  start_collectors chan logchan (Hosts x y z hosts []) index
start_collectors chan logchan (Hosts _ _ _ [] _) _ = do return ()
